...
def main():
    ...
    print("🚀 Initiating RBF Support Vector Machine (AST Overdrive)...")
    ...
    # Extract dense [768] embeddings matrices
    ...
    X_test_raw = np.stack(test_df["ast_features"].values)
    ...
    for fold_idx, (train_index, val_index) in enumerate(skf.split(X_trainval, y_trainval)):
        print(f"\n========================================")
        print(f"🔥 AST-SVM FOLD {fold_idx + 1} / 5")
        print(f"========================================")
        ...
        # 1. Feature Scaling (Critical step because SVM hyperplanes shatter if scales are uneven)
        scaler = StandardScaler()
        X_tr_scaled = scaler.fit_transform(X_tr)
        X_va_scaled = scaler.transform(X_va)
        X_test_scaled = scaler.transform(X_test_raw) # Crucial: Transform Test data using Training scale!

        # 2. Support Vector Machine Setup
        # RBF Kernel maps the 768 Dimensions into an infinite-dimensional space to guarantee boundary splits
        # Probability=True guarantees we output Logits compatible with the future text/video Multimodal Fusion!
        svm = SVC(kernel='rbf', C=1.0, class_weight='balanced', probability=True, random_state=config.SEED)
        
        # 3. Fit Model (Instant calculation of geometries instead of Deep Learning backprop "guessing")
    ...
    print(f"\n========================================")
    print(f"🏆 AST-SVM VALIDATED ON TEST DATASET 🏆")
