"""
HArnESS-Aggressive-Champion: Egyptian Arabic SER (The 50% Milestone Push).
Merging Train+Val to maximize intelligence density.

Backbone: microsoft/wavlm-base-plus (The 36% Champion).
Strategy: Master-Set Merging + WeightedRandomSampler.
Validation: Strict Test-Set Only.
Target: 50% Milestone.
"""
...
# ---------------------------------------------------------------------------
# LOSS: SUPCON (Geometric Consistency)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
class AggressiveChampionWavLM(nn.Module):
    ...
    def forward(self, wav, mask=None, mode="train"):
        ...
        # Restored Masked Pooling (The Key to 36%)
        if mask is not None:
...
def main():
    ...
    print("🏗️ Initializing Aggressive-Champion (Master-Set Merge)...")
    
    # LOAD AND MERGE
    train_df = pd.read_csv(config.SPLIT_CSV_DIR / "train.csv")
    val_df   = pd.read_csv(config.SPLIT_CSV_DIR / "val.csv")
    test_df  = pd.read_csv(config.SPLIT_CSV_DIR / "test.csv")
    master_df = pd.concat([train_df, val_df], ignore_index=True) # THE MERGE
    
    classes = sorted(master_df["emotion_final"].unique()); lid = {l: i for i, l in enumerate(classes)}
    for df in [master_df, test_df]: df["label_id"] = df["emotion_final"].map(lid)
    ...
    # WEIGHTED SAMPLER for Master Set
    y_master = master_df["label_id"].values
    class_weights = compute_class_weight("balanced", classes=np.unique(y_master), y=y_master)
    sample_weights = torch.tensor([class_weights[l] for l in y_master], dtype=torch.float32)
    ...
    sch = get_cosine_schedule_with_warmup(opt, num_warmup_steps=len(tr_loader), num_training_steps=len(tr_loader)*40)
    
    best_te = 0
    for epoch in range(1, 41):
        model.train(); pbar = tqdm(tr_loader, desc=f"Master Ep{epoch}")
        for b in pbar:
        ...
        print(f"🏆 Ep {epoch} | TEST Acc: {ta:.3f} | TEST F1: {tf:.3f}")
        if ta > best_te:
            best_te = ta; torch.save(model.state_dict(), config.CHECKPOINT_DIR / "aggressive_champion_best.pt")
