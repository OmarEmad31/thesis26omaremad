"""
HArnESS-Emotional-Apostle: Egyptian Arabic SER (The 50% Milestone Push).
Cross-Lingual Emotional Transfer (IEMOCAP English -> Egyptian Arabic).

Backbone: harshit345/wavlm-base-plus-finetuned (Pre-trained on English Emotion).
Strategy: Domain Adaptation for Egyptian Dialect.
Optimization: Extremely Low LR (2e-6) to preserve emotional feature maps.
Target: 50% Milestone.
"""
...
from transformers import WavLMModel, Wav2Vec2Model, get_cosine_schedule_with_warmup
...
def main():
    ...
    print("🏗️ Initializing Emotional Apostle (WavLM-IEMOCAP Weights)...")
    ...
    model = EmotionalApostle(len(classes)).to(device)
    ...
    print("\n🔥 STAGE 1: DIALECT ALIGNMENT (Warmup)")
    ...
    print("\n🚀 STAGE 2: APOSTLE FINE-TUNING (Sprinting toward 50%)")
    for param in model.wavlm.parameters(): param.requires_grad = True
    # VERY LOW LR to keep the emotional pre-training intact
    opt = torch.optim.AdamW(model.parameters(), lr=1e-6, weight_decay=0.01)
    sch = get_cosine_schedule_with_warmup(opt, num_warmup_steps=0, num_training_steps=len(tr_loader)*25)
    ...
    for epoch in range(1, 26):
        ...
        if va > best_va:
            best_va = va; torch.save(model.state_dict(), config.CHECKPOINT_DIR / "emotional_apostle_best.pt")
