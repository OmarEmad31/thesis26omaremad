"""
Final Audio SOTA v9 — The Academic SOTA
==========================================
Reverting to the exact architecture of `backup_sota_398.py` (which
hit 39.8%) but deploying the exact SOTA corrections dictated by deep
literature research on Few-Shot Speech Emotion Recognition:

1. **Backbone Upgrade**: Swapped general `WavLM` for `superb/wav2vec2-base-superb-er`
   which has natively pre-trained clustering for Human Emotion. Fits on T4 (Base size).
2. **Destructive VAD Removal**: Removed `librosa.effects.trim(top_db=30)`. Trimming
   destroys emotional pauses/breaths in 3-second crops. 
3. **True Normalization**: Replaced trim with Z-Score Amplitude Normalization.
4. **No Heavy Augmentation/Smoothing**: Using the exact batching, focal/supcon loop,
   and learning rate schedule from the user's successful 39.8% run to avoid crashes.
"""
...
# ─────────────────────────────────────────────────────────
# CONFIG (Exactly matching user's successful run)
# ─────────────────────────────────────────────────────────
SR             = 16000
MAX_LEN        = 80000        # 5 seconds
K_PER_CLASS    = 2            # Batch 14 (safe, proven in 39.8% run)
NUM_CLASSES    = 7
...
FOCAL_GAMMA    = 2.0
SUPCON_TEMP    = 0.07
SUPCON_WEIGHT  = 0.4          # Loss = 0.4*SupCon + 0.6*Focal
RARE_THRESHOLD = 50           

# SOTA UPGRADE: Native Emotion Backbone 
...
# MODEL ARCHITECTURE
# ─────────────────────────────────────────────────────────
class EmotionSOTA(nn.Module):
    def __init__(self, num_labels):
        super().__init__()
        base = AutoModel.from_pretrained(MODEL_NAME, output_hidden_states=True)
        cfg  = LoraConfig(r=8, lora_alpha=16, target_modules=["q_proj", "v_proj"], lora_dropout=0.05, bias="none")
        self.wav2vec = get_peft_model(base, cfg)
    ...
    def freeze_backbone(self):
        for n, p in self.wav2vec.named_parameters(): p.requires_grad = "lora_" in n
    ...
    def forward(self, wav, prosody):
        ...
        out  = self.wav2vec(wav, attention_mask=mask, output_hidden_states=True)
...
# ─────────────────────────────────────────────────────────
# DATASET (SOTA PREPROCESSING APPLIED HERE)
# ─────────────────────────────────────────────────────────
class AudioDataset(Dataset):
    ...
    def _load_audio(self, idx):
        ...
        try:
            # SOTA FIX: Load exact audio. NO `librosa.effects.trim`.
            wav, _ = librosa.load(self.path_map[fname], sr=SR)
            if len(wav) < SR // 2: return None, None
            
            # SOTA FIX: Z-Score Amplitude Normalization
            ...
            if len(wav) > MAX_LEN:
                ...
                yt = np.pad(wav, (0, MAX_LEN - len(wav)))
...
# ─────────────────────────────────────────────────────────
# EVALUATION (Keeping it smart and un-bloated)
# ─────────────────────────────────────────────────────────
...
def smart_eval(model, df, path_map, device, chunk=MAX_LEN, stride=SR, min_slide=5 * SR):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Eval", leave=False):
        ...
        try:
            # NO Trim here either to perfectly match training distribution
            wav, _ = librosa.load(path_map[fname], sr=SR)
            wav = (wav - np.mean(wav)) / (np.std(wav) + 1e-6)
            if len(wav) < SR // 2: continue
            
            if len(wav) > min_slide:
                logits_list = []
                for start in range(0, len(wav) - chunk + 1, stride):
                ...
                seg  = wav[:chunk] if len(wav) >= chunk else np.pad(wav, (0, chunk - len(wav)))
                p    = extract_prosody(seg)
                with torch.no_grad(), autocast("cuda"):
                    l, _ = model(torch.tensor(seg).unsqueeze(0).to(device),
                                  torch.tensor(p).unsqueeze(0).to(device))
...
def train():
    ...
    save_path  = colab_root / "final_emotion_best.pt"
    ...
    model     = EmotionSOTA(num_labels=len(lid)).to(device)
    ...
    print(f"\n============================================================")
    print(f"  PHASE 1 — Head Warmup (Frozen Wav2Vec2-ER)")
    print(f"============================================================")
    ...
    for ep in range(1, PHASE1_EPOCHS + 1):
        ...
        print(f"Ph1 Ep {ep:02d} | Loss {ep_loss/len(tr_frozen):.3f} | Val Acc {acc:.3f} | F1 {f1:.3f}")

    # PHASE 2
    model.unfreeze_lora()
    opt2 = torch.optim.AdamW([
        {"params": [p for n, p in model.wav2vec.named_parameters() if "lora_" not in n and p.requires_grad], "lr": 5e-7},
        {"params": [p for n, p in model.wav2vec.named_parameters() if "lora_" in n], "lr": 5e-6},
    ...
    print(f"\n============================================================")
    print(f"  PHASE 2 — Emotion Fine-tune (LoRA + SupCon)")
    print(f"============================================================")
    ...
    for ep in range(1, PHASE2_EPOCHS + 1):
        ...
        for b in tqdm(tr_loader, desc=f"Ph2 Ep{ep:02d}", leave=False):
            with autocast("cuda"):
                ...
                loss = (1 - SUPCON_WEIGHT) * focal(logits, b["label"].to(device)) + SUPCON_WEIGHT * supcon(proj, b["label"].to(device))
            opt2.zero_grad(); scaler.scale(loss).backward(); scaler.unscale_(opt2)
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            scaler.step(opt2); scaler.update(); sch2.step(); ep_loss += loss.item()
    ...
    # EVALUATION
    print("\n[EVAL] Smart sliding-window evaluation on Final Model...")
