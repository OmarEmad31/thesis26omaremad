"""
HArnESS-TriLayer: Egyptian Arabic SER (The 50% Individual Push).
Multi-Resolution Feature Fusion (Layers 4, 8, 12).

Backbone: WavLM-Base-Plus (Internal Fusion).
Features: Concatenation of Hidden States (Layers 4, 8, 12) -> 2304 dim.
Regularization: High Dropout (0.5) + Label Smoothing (0.1).
...
class TriLayerWavLM(nn.Module):
    def __init__(self, num_labels, model_name="microsoft/wavlm-base-plus"):
        ...
        # 3 Layers * 768 dim = 2304
        self.input_dim = 768 * 3
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(0.5), # Robust Regularization for high-dim features
            nn.Linear(self.input_dim, 512),
            nn.ReLU(),
            nn.Dropout(0.3),
    ...
    def forward(self, wav, mask=None, mode="train"):
        ...
        # 1. Extraction of Layers 4, 8, and 12 (Indices 4, 8, 12)
        # hidden_states is tuple of 13 tensors
        l4 = outputs.hidden_states[4]
        l8 = outputs.hidden_states[8]
        l12 = outputs.hidden_states[12]
        
        # 2. Sequential Fusion (Concatenation)
        fused_hidden = torch.cat([l4, l8, l12], dim=-1) # [BS, Seq, 2304]
...
def main():
    ...
    print("🏗️ Initializing Tri-Layer Multi-Lens Engine (WavLM-Base-Plus)...")
    ...
    model = TriLayerWavLM(len(classes)).to(device)
    ...
    print("\n🔥 STAGE 1: MULTI-LENS ALIGNMENT (Warmup)")
    ...
    print("\n🚀 STAGE 2: TRI-LAYER FINE-TUNING (The 50% Push)")
    ...
    for epoch in range(1, 26):
        ...
        if va > best_va:
            best_va = va; torch.save(model.state_dict(), config.CHECKPOINT_DIR / "trilayer_sota_best.pt")
