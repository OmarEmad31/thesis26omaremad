"""
HArnESS-WavLM-Large-Unconstrained: Egyptian Arabic SER (The 50% Milestone Push).
Large-Scale WavLM (300M) + Full Fine-tuning + Layer-wise Learning Rate Decay (LLRD).

Backbone: microsoft/wavlm-large (The 24-Layer Architect).
Strategy: Full Fine-tuning with LLRD (Decay=0.9).
Safety: Gradient Checkpointing (Memory Stability).
...
# ---------------------------------------------------------------------------
# ARCHITECTURE: WAVLM-LARGE (FULL)
# ---------------------------------------------------------------------------
class UnconstrainedWavLMLarge(nn.Module):
    def __init__(self, num_labels, model_name="microsoft/wavlm-large"):
        super().__init__()
        self.wavlm = WavLMModel.from_pretrained(model_name)
        self.wavlm.gradient_checkpointing_enable() # CRITICAL for Large full gradients
        
        # WavLM-Large hidden dim is 1024
    ...
    def forward(self, wav, mask=None, mode="train"):
        ...
        # Masked Mean Pooling (The proven winner)
        if mask is not None:
...
# ---------------------------------------------------------------------------
# DLR/LLRD: LAYER-WISE LEARNING RATE DECAY
# ---------------------------------------------------------------------------
def get_optimizer_params(model, base_lr, weight_decay, lr_decay=0.9):
    # Group layers for WavLM-Large (24 Layers)
    param_groups = []
    
    # 1. Classification & Projector (Highest LR)
    non_wavlm_params = [p for n, p in model.named_parameters() if "wavlm" not in n]
    param_groups.append({"params": non_wavlm_params, "lr": base_lr * 10, "weight_decay": weight_decay})
    
    # 2. WavLM Encoder Layers (Decaying LR)
    # layers are model.wavlm.encoder.layers[0...23]
    for i in range(24):
        layer_params = [p for n, p in model.wavlm.encoder.layers[i].named_parameters()]
        lr = base_lr * (lr_decay ** (23 - i)) # Layer 23 gets base_lr, Layer 0 gets base_lr * (0.9^23)
        param_groups.append({"params": layer_params, "lr": lr, "weight_decay": weight_decay})
        
    # 3. Base/Feature Extractor (Lowest LR)
    base_params = [p for n, p in model.named_parameters() if "wavlm.feature_extractor" in n or "wavlm.feature_projection" in n]
    param_groups.append({"params": base_params, "lr": base_lr * (lr_decay ** 25), "weight_decay": weight_decay})
...
def main():
    ...
    print("🏗️ Initializing WavLM-Large Unconstrained Giant (300M Parameters)...")
    ...
    model = UnconstrainedWavLMLarge(len(classes)).to(device)
    l_sup = SupConLoss(temperature=0.07); l_ce = nn.CrossEntropyLoss(weight=weights, label_smoothing=0.1)
    
    # 1. Warmup (Classifier Only)
    for param in model.wavlm.parameters(): param.requires_grad = False
    opt_warm = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-3)
    
    tr_loader = DataLoader(StableDataset(train_df, audio_map), batch_size=8, shuffle=True)
    ...
    print("\n🔥 STAGE 1: HYPER-WARMUP (Acoustic Alignment)")
    for epoch in range(1, 4):
        ...
        va, vf = evaluate(model, va_loader, device); print(f"📈 Warmup Epoch {epoch} | Val Acc: {va:.3f}")

    print("\n🚀 STAGE 2: UNCONSTRAINED LLRD FINE-TUNING (The Nuclear Push)")
    for param in model.wavlm.parameters(): param.requires_grad = True
    
    # Apply Layer-wise Learning Rate Decay
    opt_params = get_optimizer_params(model, base_lr=2e-5, weight_decay=0.01, lr_decay=0.90)
    ...
    for epoch in range(1, 26):
        ...
        if va > best_va:
            best_va = va; torch.save(model.state_dict(), config.CHECKPOINT_DIR / "wavlm_large_final.pt")
