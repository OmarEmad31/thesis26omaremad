"""
Egyptian Arabic Text SER — Anti-Memorization Suite (v12)
========================================================
Designed to prevent Train Acc 1.00 by utilizing augmented data,
aggressive dropout (0.5), and heavy weight decay (0.1).
"""
...
def main():
    ...
    # 1. USE AUGMENTED DATA (1113 samples to prevent memorization)
    ...
    for fold, (t_idx, v_idx) in enumerate(skf.split(texts, labels)):
        ...
        # 2. AGGRESSIVE DROPOUT (0.5 instead of 0.3)
        ...
        # 3. AGGRESSIVE WEIGHT DECAY (0.1 instead of 0.01)
        optimizer = torch.optim.AdamW([
            {'params': [p for n, p in model.named_parameters() if "bert" in n], 'lr': 2e-5, 'weight_decay': 0.1},
            {'params': [p for n, p in model.named_parameters() if "bert" not in n], 'lr': 5e-4, 'weight_decay': 0.1}
        ])
        
        criterion = nn.CrossEntropyLoss(label_smoothing=0.1) # Extra smoothing
        
        best_f1 = 0
        path = f"best_anti_mem_fold_{fold}.pt"
