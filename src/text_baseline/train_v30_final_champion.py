"""
Egyptian Arabic Text SER — The Final Champion (v30)
==================================================
The absolute definitive version for the thesis report.
Restores the 59% high-score brain of v23/v26 but changes 
the early stopping monitor to Accuracy to satisfy thesis requirements.

Specs (IDENTICAL TO v23 CHAMPION):
- Triple Pooling: [CLS] + Mean + Max.
- Multi-Sample Dropout: 5 parallel heads.
- Learning Rates: 2e-5 BERT, 8e-4 HEAD.
- Label Smoothing: 0.08.

Upgrades:
- Monitor: Early stopping now tracks 'Validation Accuracy' (VAcc).
- Reporting: Detailed MEAN CV VAL ACC + MEAN CV VAL F1 report.
"""
...
def main():
    ...
    print("\n" + "="*60 + "\n🚀 V30: THE FINAL CHAMPION (RESTORED 59% BRAIN)\n" + "="*60)
    ...
    for fold, (t_idx, v_idx) in enumerate(skf.split(texts, labels)):
        ...
        model = ChampionModel().to(device)
        optimizer = torch.optim.AdamW([
            {'params': [p for n, p in model.named_parameters() if "classifier" in n], 'lr': 8e-4}, # v23 Head LR
            {'params': [p for n, p in model.named_parameters() if "bert" in n], 'lr': 2e-5} # v23 BERT LR
        ], weight_decay=0.01)
        
        criterion = nn.CrossEntropyLoss(label_smoothing=0.08)
        best_acc, saved_f1, patience, patience_counter = 0, 0, 8, 0
        path = f"best_v30_fold_{fold}.pt"
        ...
        for epoch in range(1, 40):
            ...
            for batch_data, batch_labels in tqdm(t_loader, desc=f"   E{epoch}", leave=False):
                ...
                loss = criterion(logits, targets); loss.backward(); optimizer.step()
            ...
            # MONITOR SWITCH: Track Acc instead of F1
            if v_acc > best_acc:
                best_acc, saved_f1 = v_acc, v_f1; torch.save(model.state_dict(), path); patience_counter = 0
            else:
                patience_counter += 1
                if patience_counter >= patience: 
                    print(f"   🛑 Early stop. Best Val Acc: {best_acc:.4f}")
                    break
        
        best_val_f1s.append(saved_f1); best_val_accs.append(best_acc)
    ...
    print("\n\n" + "="*60 + "\n🏁 FINAL PHOSPHOROUS THESIS REPORT (V30 CHAMPION)\n" + "="*60)
