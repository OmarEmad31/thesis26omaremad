"""
Egyptian Arabic Text SER — Authentic Elite (v21)
==============================================
Goal: Achieve 50%+ Test Accuracy and 60%+ Validation Accuracy 
WITHOUT using augmented data or leakage.

Strategy:
- High Dropout (0.6) to destroy strings-memorization.
- High Weight Decay (0.05) to keep weights general.
- Mean Pooling + LoRA.
- Early Stopping (Patience 7) to catch the true generalization peak.
- SWA (Stochastic Weight Averaging) for smoother, higher metrics.
"""
...
# ARCHITECTURE
# ─────────────────────────────────────────────────────────
class AuthenticEliteModel(nn.Module):
    def __init__(self, model_name="UBC-NLP/MARBERT", num_labels=7):
        ...
        self.head = nn.Sequential(
            ...
            nn.LayerNorm(512),
            nn.LeakyReLU(0.1),
            nn.Dropout(0.6), # EXTREME REGULARIZATION
...
def main():
    ...
    for fold, (t_idx, v_idx) in enumerate(skf.split(texts, labels)):
        ...
        model = AuthenticEliteModel().to(device)
        optimizer = torch.optim.AdamW([
            {'params': [p for n, p in model.named_parameters() if "bert" in n], 'lr': 1e-5}, # STABLE BACKBONE
            {'params': [p for n, p in model.named_parameters() if "bert" not in n], 'lr': 3e-4}
        ], weight_decay=0.05)
        
        criterion = nn.CrossEntropyLoss(label_smoothing=0.1) # INCREASED SMOOTHING
        
        best_f1 = 0
        patience, patience_counter = 7, 0
        path = f"best_v21_fold_{fold}.pt"
