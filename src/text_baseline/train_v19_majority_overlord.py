"""
Egyptian Arabic Text SER — Majority Overlord (v19)
==================================================
The 'Accuracy First' strategy. 
Fear and Surprise are sacrificed to prioritize the 37/44 majority samples.

Techniques:
- Standard CrossEntropy (No weights, No Focus on minority).
- Mean Pooling architecture (The Author's Choice).
- LoRA Attention Adapters.
- Differential LRs & Layer-wise Decay.
...
# ─────────────────────────────────────────────────────────
class MajorityExpertModel(nn.Module):
    def __init__(self, model_name="UBC-NLP/MARBERT", num_labels=7):
        ...
        self.head = nn.Sequential(
            ...
            nn.LayerNorm(512),
            nn.GELU(),
            nn.Dropout(0.3), # SWEET SPOT DROPOUT
    ...
    def forward(self, input_ids, attention_mask):
        ...
        return self.head(sum_emb / sum_mask)
...
def main():
    ...
    print("\n" + "="*50 + "\n🚀 V19: MAJORITY OVERLORD (ACCURACY FIRST)\n" + "="*50)
    ...
    for fold, (t_idx, v_idx) in enumerate(skf.split(texts, labels)):
        ...
        model = MajorityExpertModel().to(device)
        ...
        # STANDARD CROSS ENTROPY (SACRIFICING MINORITY)
        criterion = nn.CrossEntropyLoss(label_smoothing=0.05)
        
        best_f1 = 0
        patience, patience_counter = 5, 0
        path = f"best_v19_fold_{fold}.pt"
        ...
        print(f"\n   🔥 ROLLING ENSEMBLE TEST ACC: {accuracy_score(te_labels, ensemble_preds):.4f}")
        if accuracy_score(te_labels, ensemble_preds) >= 0.50:
