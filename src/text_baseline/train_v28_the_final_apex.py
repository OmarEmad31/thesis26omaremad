"""
Egyptian Arabic Text SER — The Final Apex (v28)
==============================================
The ultimate evolution of the 59% champion (v23/v26).
Integrates Quad-Pooling, PolyLoss, and Radical Discriminative LRs.

Upgrades:
- Quad-Pooling: [CLS] + Mean + Max + STD Pooling.
- PolyLoss: Advanced loss for high-precision on small datasets.
- Discriminative LRs: 1e-5 (Lower) / 3e-5 (Upper) / 1e-3 (Head).
- 10-Sample MSD: Max-stability dropout heads.
"""
...
# ─────────────────────────────────────────────────────────
# LOSS: POLYLOSS (The Apex Upgrade)
# ─────────────────────────────────────────────────────────
class PolyLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ce_loss = F.cross_entropy(logits, targets, reduction='none', label_smoothing=0.08)
        pt = torch.exp(-ce_loss)
        poly_loss = ce_loss + self.epsilon * (1 - pt)
        return poly_loss.mean()

# ─────────────────────────────────────────────────────────
# ARCHITECTURE (QUAD POOLING + 10-MSD)
# ─────────────────────────────────────────────────────────
class ApexModel(nn.Module):
    def __init__(self, model_name="UBC-NLP/MARBERT"):
        ...
        self.dropouts = nn.ModuleList([nn.Dropout(0.3) for _ in range(10)]) # 10 heads
    ...
    def forward(self, input_ids, attention_mask):
        ...
        combined = torch.cat([cls_t, mean_p, max_p, std_p], dim=1) # [B, 3072]
...
def main():
    ...
    print("\n" + "="*60 + "\n🚀 V28: THE FINAL APEX (QUAD POOLING + POLYLOSS)\n" + "="*60)
    ...
    for fold, (t_idx, v_idx) in enumerate(skf.split(texts, labels)):
        ...
        model = ApexModel().to(device)
        # RADICAL DISCRIMINATIVE LR (LLRD-ish)
    ...
    print("\n\n" + "="*60 + "\n🏁 FINAL PHOSPHOROUS THESIS REPORT (V28 APEX)\n" + "="*60)
