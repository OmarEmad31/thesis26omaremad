"""
Egyptian Arabic Text SER — Restored Masterpiece (v26)
===================================================
A 1:1 restoration of the v23 "Masterpiece" brain that achieved 
the 59% test record, but with professional thesis reporting added.

Specs (IDENTICAL TO v23):
- Triple Pooling: [CLS] + Mean + Max.
- Multi-Sample Dropout: 5 parallel heads.
- Learning Rates: 2e-5 BERT, 8e-4 HEAD.
- Label Smoothing: 0.08.
- Added Upgrade: Final Comprehensive Thesis Report (Mean Val Acc, etc).
"""
...
# ─────────────────────────────────────────────────────────
# ARCHITECTURE (V23 MASTERPIECE)
# ─────────────────────────────────────────────────────────
class RestoredModel(nn.Module):
...
def main():
    ...
    print("\n" + "="*60 + "\n🚀 V26: RESTORED MASTERPIECE (THE 59% BACKBONE)\n" + "="*60)
    ...
    for fold, (t_idx, v_idx) in enumerate(skf.split(texts, labels)):
        ...
        model = RestoredModel().to(device)
        optimizer = torch.optim.AdamW([
            {'params': [p for n, p in model.named_parameters() if "classifier" in n], 'lr': 8e-4}, # RESTORED TO v23 SPEED
            {'params': [p for n, p in model.named_parameters() if "bert" in n], 'lr': 2e-5}
        ], weight_decay=0.01)
        
        criterion = nn.CrossEntropyLoss(label_smoothing=0.08) # RESTORED TO v23 SMOOTHING
        best_f1, best_acc, patience, patience_counter = 0, 0, 7, 0
        path = f"best_v26_fold_{fold}.pt"
    ...
    print("\n\n" + "="*60 + "\n🏁 FINAL PHOSPHOROUS THESIS REPORT (V26 RESTORED)\n" + "="*60)
